...
class _GoldViewState extends State<GoldView> {
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      setState(() {
        _isVisible =
            _scrollController.offset < 100; // Adjust threshold as needed
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => GoldCubit(getIt.get<GoldRepo>()),
      child: Scaffold(
        body: NestedScrollView(
          controller:
              _scrollController, // Keep the controller for tracking scroll
